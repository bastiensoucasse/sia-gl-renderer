...
public:
    ...
    /// Add a
    void addQuad(const Vector4f &p0, const Vector4f &p1, const Vector4f &p2, const Vector4f &p3);
...
private:
    ...
    /**
     * @brief Gets the four shadow volume quad points positions from a halfedge.
     *
     * @param halfedge The halfedge to use to compute the quad.
     * @param lightPosition The position of the current light source.
     * @return Eigen::Vector4f* The positions of the four quad points as an array.
     */
    Eigen::Vector4f *getShadowVolumeQuadPoints(const pmp::Halfedge &halfedge, const Eigen::Vector3f &lightPosition) const;

    /**
     * @brief Checks if a face is enlightened by a light source.
     *
     * @param face The face to analyse.
     * @param lightPosition The position of the current light source.
     * @return True if the face is enlightened bu the current light source;
     * @return false otherwise.
     */
    bool is_enlightened(const pmp::Face &face, const Eigen::Vector3f &lightPosition) const;
};
