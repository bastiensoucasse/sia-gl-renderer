...
void Mesh::createSphere(float radius, int nU, int nV)
{
    ...
    for (int v = 0; v <= nV; ++v)
    {
        for (int u = 0; u <= nU; ++u)
        {
            ...
            normals.col(idx) << sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta);
...
void Mesh::updateVBO()
{
    ...
    VertexProperty<Vector4f> vertices = get_vertex_property<Vector4f>("v:position");
    size_t n_vertices = vertices.vector().size();
    if (vertices)
    {
        glBindBuffer(GL_ARRAY_BUFFER, _vbo[VBO_IDX_POSITION]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(Vector4f) * n_vertices, vertices.vector()[0].data(), GL_STATIC_DRAW);
    }
    ...
    if (vnormals)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(Normal) * n_vertices, vnormals.vector()[0].data(), GL_STATIC_DRAW);
    }

    VertexProperty<TexCoord> texcoords = get_vertex_property<TexCoord>("v:texcoord");
    if (texcoords)
    {
        glBindBuffer(GL_ARRAY_BUFFER, _vbo[VBO_IDX_TEXCOORD]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(TexCoord) * n_vertices, texcoords.vector()[0].data(), GL_STATIC_DRAW);
    }
    ...
    if (colors)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(Color) * n_vertices, colors.vector()[0].data(), GL_STATIC_DRAW);
    }
...
/**
 * @brief Add the quad to the mesh.
 *
 * p0 ---- p1
 * |     / |
 * |  /    |
 * p2 ---- p3
 */
void Mesh::addQuad(const Vector4f &p0, const Vector4f &p1, const Vector4f &p2, const Vector4f &p3)
{
...
Mesh *Mesh::computeShadowVolume(const Vector3f &lightPos, bool interpolated)
{
    ...
    shadowVolume->add_vertex_property<Vector4f>("v:position");

    for (SurfaceMesh::FaceIterator fit = faces_begin(); fit != faces_end(); ++fit)
    {
        Face face = *fit;

        if (is_boundary(face))
        {
            Halfedge currentHalfedge = halfedge(face);

            do
            {
                if (is_boundary(currentHalfedge))
                {
                    Vector4f *quadPoints = getShadowVolumeQuadPoints(currentHalfedge, lightPos);
                    shadowVolume->addQuad(quadPoints[0], quadPoints[1], quadPoints[2], quadPoints[3]);
                }

                currentHalfedge = next_halfedge(currentHalfedge);
            } while (currentHalfedge != halfedge(face));
        }
        else if (is_enlightened(face, lightPos))
        {
            Halfedge currentHalfedge = halfedge(face);

            do
            {
                Face oppositeFace = this->face(opposite_halfedge(currentHalfedge));

                if (!is_enlightened(oppositeFace, lightPos))
                {
                    Vector4f *quadPoints = getShadowVolumeQuadPoints(currentHalfedge, lightPos);
                    shadowVolume->addQuad(quadPoints[0], quadPoints[1], quadPoints[2], quadPoints[3]);
                }
...
Vector4f *Mesh::getShadowVolumeQuadPoints(const Halfedge &halfedge, const Vector3f &lightPosition) const
{
    const auto verticesPositions = get_vertex_property<Vector4f>("v:position");

    static Vector4f quadPoints[4];
    quadPoints[0] = verticesPositions[from_vertex(halfedge)];
    quadPoints[1] = verticesPositions[to_vertex(halfedge)];
    quadPoints[2] << quadPoints[0].head<3>() - lightPosition, 0.;
    quadPoints[3] << quadPoints[1].head<3>() - lightPosition, 0.;

    return quadPoints;
}

bool Mesh::is_enlightened(const Face &face, const Vector3f &lightPosition) const
{
    const auto verticesPositions = get_vertex_property<Vector4f>("v:position");
    const auto faceNormals = get_face_property<Normal>("f:normal");

    const Vector3f facePosition = verticesPositions[from_vertex(halfedge(face))].head<3>();
    const Vector3f faceNormal = Vector3f(faceNormals[face]).normalized();

    return (lightPosition - facePosition).dot(faceNormal) > 0;
}
